...
def prepare_data(target_site, bucket='niwa-water-demand-modelling', save_local=True):
    ...
    df = add_lag_features(df, target_col, site_name=target_site)
    
    # Save locally if requested
    if save_local:
        local_dir = 'data/processed'
        os.makedirs(local_dir, exist_ok=True)
        filename = target_site.replace(")", "").replace("(", "").replace(" ", "_")
        local_path = f"{local_dir}/{filename}_with_lags.csv"
...
def main(run_acf=True, run_ablation=True, run_benchmark=True, run_importance=True, run_ensemble=False):
    ...
    acf_variables = ['Tmax', 'Sun', 'PE', 'Aquifer', 'API', 'storage']
    
    all_results = {}
    
    for site in target_sites[1:]:
        print(f"\n{'='*60}")
        print(f"Processing: {site}")
        print(f"{'='*60}")
        
        df, target_col = prepare_data(site)
        site_results = {}
        ...
        if run_ablation:
            ...
            print(f"   ({split_method} split, {train_size:.0%} train, testing 7 algorithms)")
            ...
            ablation_results.to_csv(f'{results_dir}/ablation_{site}.csv', index=False)
            site_results['ablation'] = ablation_results
            ...
            summary_cols = ['feature_group', 'n_features', 'n_effective', 'RF_rmse', 'RF_mae', 'RF_r2', 
                           'RF_eff', 'GB_eff', 'CB_eff', 'XGB_eff', 'ET_eff', 'MLP_eff', 'LR_eff', 'consensus']
            ...
            print(f"\n   Consensus: {n_consensus}/{n_total} groups effective (>50% of 7 algorithms agree)")
        
        # 3. Algorithm Benchmark
        if run_benchmark:
            # Try to load ablation results from file if not in memory
            ablation_df = None
            if run_ablation and 'ablation' in site_results:
                ablation_df = site_results['ablation']
            else:
                # Try loading from saved file
                ablation_file = f'{results_dir}/ablation_{site}.csv'
            ...
            algo_results.to_csv(f'{results_dir}/algorithms_{site}.csv', index=False)
            site_results['algorithms'] = algo_results
        ...
        if run_importance:
            ...
            importance = feature_importance_analysis(df, target_col, all_features, train_size, split_method)
            importance.to_csv(f'{results_dir}/importance_{site}.csv', index=False)
            site_results['importance'] = importance
        ...
        if run_ensemble:
            ...
            ensemble_metrics.to_csv(f'{results_dir}/ensemble_metrics_{site}.csv', index=False)
            site_results['ensemble'] = ensemble_result
