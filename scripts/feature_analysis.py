...
def run_acf_analysis(df, target_col, variables, max_lag=30, save_dir='results/acf', 
    ...
    for var in [target_col] + variables:
        ...
        # Count how many of first 10 lags are outside confidence band
        significant_lags_orig = np.sum(np.abs(acf_orig[1:11]) > conf_band) if len(acf_orig) > 10 else 0
        # If >7 out of 10 lags significant, series has strong persistence
        acf_decays = significant_lags_orig < 7
        ...
        significant_lags_diff = np.sum(np.abs(acf_diff[1:11]) > conf_band) if len(acf_diff) > 10 else 0
        acf_diff_decays = significant_lags_diff < 7
...
def define_feature_groups(site_name=None):
    ...
    groups = {
        'date_basic': ['doy', 'month', 'mday', 'wday'],
        'season': ['Season'],
        'holiday': ['is_holiday'],
        'tmax': ['Tmax'],
        'tmax_lag1': ['Tmaxlag1'],
        'tmax_lag2_3': ['Tmaxlag2', 'Tmaxlag3'],
        'sun': ['Sun'],
        'sun_lag1': ['Sunlag1'],
        'sun_lag2_3': ['Sunlag2', 'Sunlag3'],
        'pe': ['PE'],
        'pe_lag1_2_3': ['PElag1', 'PElag2', 'PElag3'],
        'rain': ['Aquifer', 'WVRain', 'KerburnRain'],
        'rain_lag1_2_3': ['Rainlag1', 'Rainlag2', 'Rainlag3'],
        'rain_rolling': ['Rain_L7DAYS', 'Rain_L6DAYS', 'Rain_L5DAYS', 'Rain_L4DAYS', 
                         'Rain_L3DAYS', 'Rain_L2DAYS'],
        'cyclical': ['ANcyc'],
        'api': ['API'],
        'api_lag1': ['lagAPI'],
        'api_lag2_4': ['lag2API', 'lag4API'],
        'storage': ['storage'],
        'storage_lag1': ['Storagelag1'],
        'storage_lag2_4': ['Storagelag2', 'Storagelag4'],
        'restriction': ['Restriction level']
    }
    
    # Add site-specific features if site name provided
    if site_name:
        groups['ftemp'] = [f'{site_name} fTemp']
        groups['fprecp'] = [f'{site_name} fPrecp']
        groups['cm'] = [f'{site_name} cm']
        groups['stat'] = [f'{site_name} stat']
        groups['stat_lag1_2_3'] = [f'{site_name} statlag1', f'{site_name} statlag2', f'{site_name} statlag3']
    else:
        groups['ftemp'] = ['fTemp']
        groups['fprecp'] = ['fPrecp']
        groups['cm'] = ['cm']
        groups['stat'] = ['stat']
        groups['stat_lag1_2_3'] = ['statlag1', 'statlag2', 'statlag3']
    
    # Target lags MUST be last
    groups['target_lag1_2_3'] = ['Targetlag1', 'Targetlag2', 'Targetlag3']
...
def feature_ablation_study(df, target_col, feature_groups, train_size=0.7, split_method='stratified'):
    ...
    # Test algorithms for consensus
    test_algorithms = {
        'RF': RandomForestRegressor(random_state=42, n_estimators=100),
        'GB': GradientBoostingRegressor(random_state=42, n_estimators=100),
        'CB': CatBoostRegressor(random_state=42, iterations=100, verbose=0),
        'XGB': XGBRegressor(random_state=42, n_estimators=100),
        'ET': ExtraTreesRegressor(random_state=42, n_estimators=100),
        'MLP': MLPRegressor(random_state=42, max_iter=500, early_stopping=True),
        'LR': LinearRegression()
    }
    ...
    for group_name, features in feature_groups.items():
        ...
        # Consensus: >50% of algorithms show improvement
        n_algos = len(test_algorithms)
        consensus = sum(improvements) > n_algos / 2
...
def feature_importance_analysis(df, target_col, features, train_size=0.7, split_method='stratified'):
...
def train_stacked_ensemble(df, target_col, features, n_folds=5, train_size=0.7, split_method='stratified', random_state=42, save_dir=None, site_name=None):
    ...
    layer1_models = {
        'XGB': XGBRegressor(random_state=random_state, n_estimators=100),
        'GB': GradientBoostingRegressor(random_state=random_state, n_estimators=100),
        'CB': CatBoostRegressor(random_state=random_state, iterations=100, verbose=0),
        'RF': RandomForestRegressor(random_state=random_state, n_estimators=100),
        'ET': ExtraTreesRegressor(random_state=random_state, n_estimators=100),
    ...
    if save_dir:
        ...
        # Save ensemble model
        model_name = f"ensemble_{site_name}.pkl" if site_name else "ensemble_model.pkl"
        ...
        pred_name = f"predictions_{site_name}.csv" if site_name else "predictions.csv"
        ...
        # Plot time series
        plot_name = f"timeseries_{site_name}.png" if site_name else "timeseries.png"
        ...
        ax1.plot(train_dates, train_actual, label='Actual', alpha=0.7, linewidth=1)
        ax1.plot(train_dates, train_pred, label='Predicted', alpha=0.7, linewidth=1)
        ax1.set_title(f'Training Set - R² = {train_r2:.4f}')
        ax1.set_ylabel('Value')
        ax1.legend()
        ax1.grid(True, alpha=0.3)
        
        ax2.plot(test_dates, test_actual, label='Actual', alpha=0.7, linewidth=1)
        ax2.plot(test_dates, test_pred, label='Predicted', alpha=0.7, linewidth=1)
        ...
        # Plot scatter
        scatter_name = f"scatter_{site_name}.png" if site_name else "scatter.png"
